...
async function generateHiringRadarLeadMagnet() {
  ...
  // Fallback if seed file is not directly readable
  if (!companies.length) {
    companies = [
      { company: 'Stripe', domain: 'stripe.com', careersUrl: 'https://stripe.com/jobs', atsType: 'Greenhouse', activeJobCount: 42, sampleRoles: 'Staff Distributed Systems Engineer, Product Manager, Enterprise AE', location: 'Remote / US' },
      { company: 'Datadog', domain: 'datadoghq.com', careersUrl: 'https://careers.datadoghq.com', atsType: 'Greenhouse', activeJobCount: 38, sampleRoles: 'Senior SRE, Solutions Architect, Technical Recruiter', location: 'New York / Remote' },
      ...
      { company: 'Notion', domain: 'notion.so', careersUrl: 'https://notion.so/careers', atsType: 'Ashby', activeJobCount: 24, sampleRoles: 'Infrastructure Engineer, Growth PM, Engineering Manager', location: 'San Francisco / New York' },
      { company: 'Ramp', domain: 'ramp.com', careersUrl: 'https://ramp.com/careers', atsType: 'Ashby', activeJobCount: 22, sampleRoles: 'Senior Software Engineer (Risk), Product Marketing Lead', location: 'New York / Remote' },
    ];
  }
  ...
  const mdContent = `# 📡 Live Verified Hiring Radar (${dateStr})
...
| :--- | :--- | :--- | :--- | :--- |
${companies.map(c => `| **${c.company}** | **${c.activeJobCount} roles** | ${c.sampleRoles} | \`${c.atsType}\` | [Apply Direct](${c.careersUrl}) |`).join('\n')}

---
💡 **Pro Tip for Job Seekers:** Don't submit cold applications into the black hole. 
Use [BD Engine](https://bd-engine-production.up.railway.app/job-search) to match your 1st-degree LinkedIn connections to these exact active companies and generate grounded warm intro notes in 1 click.
`;
